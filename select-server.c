...
// instructs the loop about the next steps for the fd for which the callback was invoked.
...
fd_status_t on_peer_connected(int sockfd, const struct sockaddr_in* peer_addr, socklen_t peer_addr_len){
    ...
    //send signal that socket is ready for writing
    return fd_status_W;
}
...
fd_status_t on_peer_ready_recv(int sockfd){
    ...
    if(nbytes == 0){
        ...
        if(errno == EAGAIN || errno == EWOULDBLOCK){
            //socket not ready for recv, wait
            return fd_status_R;
        }
        else{
            perror_die("recv");
...
fd_status_t on_peer_ready_send(int sockfd){
    ...
    if(nsent < sendlen){
        ...
        //everything sent successfully, reset the send queue
        peerstate->sendptr = 0;
